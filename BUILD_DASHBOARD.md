...
- Polish: reads the NVIDIA key the Nova tile saves to `localStorage` (`nova_lite_api_key`). If empty → fall back to plain Add and show a brief tertiary message in `statusEl` saying `Polish needs an NVIDIA API key (set on the Nova tile) — added as-typed.` for 3.5 seconds. If a key is set → POST to `/api/nova-proxy` (a server-side relay to NVIDIA's NIM API, since NVIDIA doesn't support direct browser CORS) with `{ key, messages }`, asking model `moonshotai/kimi-k2.6` to clean up exactly ONE goal and return it as a one-element JSON array of strings (no preamble, no fences). Parse, push, save, clear input. On any error: add the raw text and show `Polish failed — added as-typed.` in red for 3.5s.
